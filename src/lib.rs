    ...
    impl XoaiElement {
        ...
        fn from_node(node: &roxmltree::Node) -> err::Result<Self> {
            ...
            let fields = node.children()
                ...
                .map(|x| {
                    (x.attribute("name").map(String::from), x.text().map(String::from))
                })
                .collect::<Vec<_>>();
...
pub mod list_records {
    ...
    pub struct ListRecords<T, U> {
        ...
        pub records: Vec<record::Record<U>>,
    ...
    impl<T: pref::MetadataPrefix> ListRecords<T, T::MetadataType> {

        pub fn has_next(&self) -> bool {
            self.resumption_token.as_ref().and_then(|x| x.value.as_ref()).is_some()
        }

        pub fn get_next(self, client: &Client) -> impl Future<Item = ListRecords<T, T::MetadataType>, Error = err::Error> {
            if let Some(rt) = self.resumption_token.as_ref()
        ...
        fn build(text: String) -> err::Result<ListRecords<T, T::MetadataType>> {
            ...
            let resumption_token = list_records.find_first_child("resumptionToken").ok()
                .and_then(|x| {
                    match (
                        x.text(),
    ...
    impl Client {
        pub fn list_records<T: pref::MetadataPrefix>(
            ...
            let rt = self
                ...
                .and_then(|text| ListRecords::<T, T::MetadataType>::build(text).into_future().from_err());

            EitherFuture::A(rt)
        }

        pub fn list_records_from_resumption_token<T: pref::MetadataPrefix>(
            &self, resumption_token: String
        ) -> impl Future<Item = ListRecords<T, T::MetadataType>, Error = err::Error> {
        ...
        pub fn list_records_all<T: pref::MetadataPrefix>(
            &self, params: Option<Params>
        ) -> impl Future<Item = ListRecords<T, T::MetadataType>, Error = err::Error> {

            fn run<T: pref::MetadataPrefix>(client: Client, params: Option<Params>)
                -> impl Future<Item = ListRecords<T, T::MetadataType>, Error = err::Error> {
                ...
                loop_fn(EitherFuture::A(client.list_records(params)), move |list_records_fut| {
                    ...
                    list_records_fut.and_then(move |mut list_records| {
                        if list_records.has_next() {
                            ...
                            let rt = list_records.get_next(&client)
                                .map(|mut x| { 
...
mod ext {
    ...
    pub(crate) trait NodeExt<'a, 'd>: Sized {
        fn find_first_child<I: Into<ExpandedName<'a>>>(&self, tag_name: I)
            -> crate::err::Result<Node<'a, 'd>>;
        fn find_first_child_parsed_text<P: std::str::FromStr, I: Into<ExpandedName<'a>>>(&'a self, tag_name: I)
            -> crate::err::Result<P>;
    }
